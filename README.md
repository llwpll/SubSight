...
从 GitHub Releases 下载 `SubSight-<version>-macos-app.zip`，解压后打开 App：
...
从 GitHub Releases 下载 `subsightctl-<version>-macos-<arch>.tar.gz`，解压并安装到 `PATH` 中的目录：
...
打开 `SubSight.app` 后，可以点击右上角的添加按钮录入订阅；App 和 CLI 会共用同一个本地数据文件。
